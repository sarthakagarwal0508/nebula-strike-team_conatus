...
function createEnemy(typeName) {
    ...
    enemyState.enemies.push({
        ...
        hp: data.hp,
        speed: data.speed +
            Math.min(0.08, enemyState.wave * 0.01),
        points: data.points,
...
function updateEnemies() {
    ...
    for (
        ...
        if (enemy.type === "shooter")
            enemy.x +=
                Math.sin(
                    (enemy.y + enemy.drift) * 0.08
                ) * 0.15;

        if (enemy.type === "tank")
            enemy.x +=
                Math.sin(
                    (enemy.y + enemy.drift) * 0.04
                ) * 0.08;

        if (enemy.type === "bomber")
            enemy.x +=
                (player.x - enemy.x) * 0.0015;
...
function removeEnemyBullet(bullet) {
    ...
    const index =
        enemyState.bullets.indexOf(bullet);
...
function removeEnemy(enemy, reward = true) {
    ...
    const index =
        enemyState.enemies.indexOf(enemy);
...
function updatePowerUps() {
    ...
    for (
        ...
        if (
            ...
            window.NebulaGame.collectPowerUp(
                power.type
            );
